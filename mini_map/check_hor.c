...
/*   Updated: 2023/06/17 12:06:38 by rakhsas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_intercepts(t_data *data)
{
	...
	if (!check_hit_hor(data->map->map_copy,
			data->cords.xinterc_hor, data->cords.yinterc_hor))
	{
...
int	get_second_hor_cord(t_data *data)
{
	...
	if (!check_left)
		data->cords.xb_hor = data->cords.xinterc_hor
			+ fabs(CARE / tan(data->player.ray_angle));
	else
		data->cords.xb_hor = data->cords.xinterc_hor
			- fabs(CARE / tan(data->player.ray_angle));
	...
	if (!check_hit_hor(data->map->map_copy,
			data->cords.xb_hor, data->cords.yb_hor))
		return (1);
	return (0);
}
